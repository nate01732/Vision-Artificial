...
for clase in clases:
    for idx, imagen in enumerate(arreglos_imagenes[clase]):
        if idx >= 2:  # Limitar a dos imágenes por clase para la visualización
            break
        ...
        if len(contornos) > 0:
            ...
            axs[2].imshow(bordes_unidos, cmap='gray')
            axs[2].set_title('Bordes Unidos')
            axs[2].axis('off')

            axs[3].imshow(imagen_segmentada)
