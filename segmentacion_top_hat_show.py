...
for clase in clases:
    for idx, imagen in enumerate(arreglos_imagenes[clase]):
        if idx >= 2:  # Limitar a dos imágenes por clase
            break

        imagen_np = np.array(imagen)

        # Realzar nitidez
        imagen_nitida = realce_nitidez(imagen)

        # Detectar bordes
        bordes = detectar_bordes(imagen_nitida)
        ...
        # Guardar la imagen segmentada en RGB
        ...
        fig.suptitle(f"Proceso de Segmentación para {clase} - Imagen {idx + 1}")

        axs[0].imshow(imagen)
        axs[0].set_title('Original')
        axs[0].axis('off')

        axs[1].imshow(imagen_nitida)
        axs[1].set_title('Nitidez Mejorada')
        axs[1].axis('off')
        ...
        axs[3].imshow(bordes_unidos, cmap='gray')
        axs[3].set_title('Bordes Unidos')
        axs[3].axis('off')

        axs[4].imshow(umbral, cmap='gray')
        axs[4].set_title('Máscara de Contorno')
        axs[4].axis('off')

        axs[5].imshow(imagen_segmentada)
        axs[5].set_title('Segmentada')
        axs[5].axis('off')
